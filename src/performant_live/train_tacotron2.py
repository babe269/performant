...
formant_record = np.empty((0,5), int)
formant_intervals = [23007, 23014]
...
class Tacotron2Trainer(Seq2SeqBasedTrainer):
    ...
    def do_synthesis(self, input_text, text2mel_model, vocoder_model, processor, text2mel_name, vocoder_name):
    ...
    def compute_formants(self, models, path ,vowels, language, config_tf, config_mel, processor_path):
      import sys
      import numpy as np
      ...
      data = []
      count = 0
      for index,model in enumerate(models): #model level
        vl = []
        f1l = []
        f2l = []
        #HVD sentences generated for obtaining vowel data. 
        HVD = ["Heard...foot...hud...heed...head...had...hard...hod...thought...goose...heard...hid.",
              "Foot...hod...heard...heed...hard...had...hid...thought...goose...head...hod...hud.",
              "Hud...head...thought...had...heed...hod...goose...hid...hard...heard...head...foot.",
              "Hud...hod...hid...hard...heard...had...goose...head...thought...heed...hod...foot.",
              "Foot...hard...hud...hid...heard...goose...thought...hod...hard...had...head...heed."]

        processor = AutoProcessor.from_pretrained(pretrained_path=processor_path)
        path2model = path + "/model-" + model +".h5"
        tacotron2 = TFAutoModel.from_pretrained(
          config=config_tf,
          pretrained_path= path2model,
          name="tacotron2")
        ...
        for sentence in HVD:
          clear = False
          #This while Loop is used to ensure that no undesired audio is synthesised using attention data. 
          while not clear:
            input_text = sentence
            tacotron2.setup_window(win_front=10, win_back=10)
            mels, alignment_history, audios = self.do_synthesis(input_text, tacotron2, melgan, processor, "TACOTRON", "MELGAN")
            ipd.display(ipd.Audio(audios, rate=22050))
            ah = alignment_history[0]
            count = 0
            for i in ah[len(ah)-1]:
                if i < 0.98:
                  count += 1
            print(len(ah[len(ah)-1]))
            print(count)
            if count/len(ah[len(ah)-1]) > 0.9:
              clear = True
          #After generating an acceptable sample store both the audio and the text transcription in current directory.
          with open(sentence + ".txt", 'w') as out:
            out.write(input_text)
          scipy.io.wavfile.write(sentence+".wav", 22050, audios)
          #Call WebMaus Basic Api to generate TextGrids.

          files = {
            'SIGNAL': (sentence + '.wav', open(sentence + '.wav', 'rb')),
            'LANGUAGE': (None, language),
            'OUTFORMAT': (None, 'TextGrid'),
            'TEXT': (sentence + '.txt', open(sentence + '.txt', 'rb')),
          }
          ...
          downURL = downURL[0]
          urllib.request.urlretrieve(downURL, sentence+".TextGrid")
          #Use parselmouth + Textgrid tools to obtain formant information 
          ps = parselmouth.Sound(sentence + ".wav")
          print(ps)
          formants = ps.to_formant_burg()
          tg = tgt.io.read_textgrid(sentence + '.TextGrid')
          print(tg)
        ...
        d = {'f1': f1Centroid, 'f2': f2Centroid, 'steps': model, 'point': index}
        df2=pd.DataFrame(d)
        data.append(df2)
      full = pd.concat(data)
      display(full)
    ...
    def _train_step(self, batch):
        ...
        for idx, interval in enumerate(formant_intervals):
          if self.steps == interval:
              models = [str(self.steps)]
              print(models)
              super(Tacotron2Trainer, self).save_checkpoint()
              ...
              processor_path = os.getcwd() + "/tensorflow_tts/processor/pretrained/ljspeech_mapper.json"
              vowels = ["{", "}:","3:","6","6:","e","I", "i:", "O", "o:", "U" ]
              path = os.getcwd() + "/examples/tacotron2/exp/train.tacotron2.v1/checkpoints"
              formant_out = self.compute_formants(models, path, vowels, "eng-NZ", tacotron2_config, melgan_config, processor_path) 
              corrected = formant_out.reset_index()
              nparr = corrected.to_numpy()
              formant_record = np.concatenate((formant_record, nparr), axis=0)
              print(formant_record)
              df = pd.DataFrame(formant_record, columns = ['vowel','f1','f2','steps', 'point'])
              display(df)
              fig = px.line(df, x="f2", y="f1",color='vowel', width=1000, height=900, line_shape= 'spline', text = 'steps', line_group='vowel', )
              fig.update_layout(
              ...
              fig.show()
              if not os.path.exists("formant_plots"):
                os.mkdir("formant_plots")
              fig.write_image("formant_plots/" + str(self.steps) +".png")
        # update counts
        self.steps += 1
        self.tqdm.update(1)
...
def main():
    ...
    with STRATEGY.scope():
        ...
        tacotron2 = TFTacotron2(config=tacotron_config,)
        tacotron2._build()
        tacotron2.summary()
